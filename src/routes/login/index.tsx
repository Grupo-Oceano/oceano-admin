...
import { checkCookieValidity } from "~/api/auth";
...
export const onGet = async ({ cookie, redirect }: RequestEvent) => {
  ...
  const isAuthorized = await checkCookieValidity(cookieValue.value);
  if (isAuthorized) {
...
export default component$(() => {
  ...
  return (
    <div class="flex flex-col gap-3 bg-transparent px-8 shadow-2xl backdrop-blur-lg lg:max-w-none lg:px-4 lg:shadow-none lg:backdrop-blur-none">
      ...
      >
        <span class="material-symbols-rounded" q:slot="iconBefore">
          login
        </span>
      </ActionButton>
