...
import { SharedMap } from "~/common/types";
...
export const useUser = routeLoader$((ev) => {
  return ev.sharedMap.get(SharedMap.USER) as User | null;
});
    ...
    <div class="grid h-screen w-screen grid-cols-10 grid-rows-10 flex-col gap-4 bg-gray-200 p-4 dark:bg-gray-800">
      ...
      <div class={["col-span-2 row-span-3", gridSectionClass]}>
        <div class="h-fill flex flex-col px-2" title="This is mocked data">
          <h3 class="mb-3 text-lg font-semibold text-gray-700 xl:text-2xl dark:text-white">
            System Status
          </h3>
          <div class="flex flex-grow flex-col justify-between px-2">
            <div class="space-y-2">
              <div class="flex items-center gap-2">
                ...
                <small class="text-sm text-gray-800 xl:text-base dark:text-gray-300">
                  All Services Online
                </small>
              </div>
              <div class="flex items-center gap-2">
            ...
            <small class="text-xs text-gray-500 xl:text-sm dark:text-gray-400">
